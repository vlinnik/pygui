...
parser.add_argument('-p','--package',action='store',default='gui',help='Имя проекта')
parser.add_argument('-u','--user',action='store',default='your_name')
parser.add_argument('-m','--mail',action='store',default='your_name@domain.com')
parser.add_argument('-l','--homepage',action='store',default='project_name')

args = parser.parse_args()

template = importlib.__import__(f'pygui.resources.{args.template}',fromlist=['createProject'])

package_dir = f'{args.destination}/{args.package}/' 
project_dir = f'{args.destination}/' 
os.makedirs(f'{package_dir}/ui',exist_ok = True )

data = resources.files('pygui.resources')
for f in ['LICENCE','README.md']:
    shutil.copy(data.joinpath(f),f'{project_dir}/{f}')
shutil.copy(data.joinpath('default.scada'),f'{package_dir}/default.scada')
